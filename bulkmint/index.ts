...
const RPC_URL = process.env.RPC_URL? process.env.RPC_URL : 'https://polygon-mumbai.infura.io/v3/7b2';
...
const ProcessNFTMetadata = async(data) => {
    try {
        ...
        for (let i = 0; i < data.length; i++) {
            ...
            console.log("Fetching NFT Image from GCP for token_id: ", data[i].token_id);

            const Image_Path = `${data[i].job_id}/${data[i].token_id}.jpg`;
            const Image = await getImage(Image_Path) || '';
            const ImageBuffer = Buffer.from(Image);
            const ImageIPFS = await uploadFile(ImageBuffer);
            console.log("Image Uploaded to IPFS: ", ImageIPFS);
            let name = data[i].name;
            let description =data[i].description;
            let category =data[i].category;
            let recipient =data[i].recipient;
            let imageURI = `https://ipfs.io/ipfs/${ImageIPFS}`;
            ...
            for (var j = 0; j < keys.length; j++) {
              var trait_type = keys[j];
              var value = JSON.stringify(metaData[trait_type]).replace(/"/g, '');
              let trait = JSON.stringify({ "trait_type": trait_type, "value": value })
              attributes.push(JSON.parse(trait));
            }

            const odHashForImage = crypto.createHash('sha3-256').update(Image).digest('hex');
            const messageHash = await web3provider.utils.sha3(odHashForImage) || '';
            let signature = (await web3provider.eth.accounts.sign(messageHash, `0x${privateKey}`)).signature;
            let signtrait = JSON.stringify({ "trait_type":   "signature", "value": signature })
            let odhashtrait = JSON.stringify({ "trait_type": "ODHash",  "value": odHashForImage })
        ...
        let uploadJson = {
            ...
            description,
            category,
            image: `https://storage.cloud.google.com/${GCP_NFTTRACE_BUCKET_NAME_IMAGE}/${Image_Path}`,
        ...
        const encodedData = factoryContract?.interface.encodeFunctionData(
            'issueCertificate',
            [contract_address, recipient, tokenId, imageURI]
        );
        ...
        params[contract_address].tokenURI.push(metadatahash);
        ...
        params[contract_address].mintTx.push({ to: FACTORY_CONTRACT_ADDRESS, data: encodedData }); 
    }
    ...
    for (const nftContract of contractAddressSet){
        ...
        console.log("Embedding Metadata for tokenId: ", params[nftContract].tokenId[i]);
        ...
        const embededMetaData = `<--NFTRACE_KEY_START-->${Buffer.from(JSON.stringify(embedData))}<--NFTRACE_KEY_END-->`;       
        const modifiedFile = Buffer.concat([params[nftContract].imageBuffer[i], Buffer.from(embededMetaData)]);
        await uploadVerifiableFile(modifiedFile, params[nftContract].imageName[i]);
    }
...
const processTxs = async(mintTxs) => {
    const paymasterUrl = "https://paymaster.biconomy.io/api/v1/80002/qX9uXTHNm.dc1e610e-daee-46a8-acb2-7b6438fd8428"
